...
def test_features():
    ...
    feature = Feature('first_feature', 'test type')
    start, end = 4, 7
    s.add_feature(start, end, feature)
    assert_equal(s.get_features()[feature], list(range(start,end+1)))

    new_feature =  s.create_feature('second_feature', 'test type', start, end)
    assert_equal(s.get_features()[new_feature], list(range(start, end+1)))


    # Find Features
    assert_equal(s.find_feature('second_feature'), set([new_feature]))
    assert_equal(s.find_feature('scond_f'), set([]))
    ...
    cut_start = 10
    cut_end = 15
    fragments = s.cut((cut_start, cut_end))

    l1 = [(x.start, x.end) for x in fragments[0].find_feature('circular_feature')]
    l2 = [(x.start, x.end) for x in fragments[1].find_feature('circular_feature')]

    x1 = (0, cut_start - start - 1)
    x2 = (cut_end-start, None)
    x3 = (cut_start - start, cut_end-start-1)

    assert_true(x1 in l1)
    assert_true(x2 in l1)
    assert_true(x3 in l2)
