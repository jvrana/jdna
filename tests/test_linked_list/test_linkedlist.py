...
@pytest.mark.parametrize("i,j", [(0, 10), (2, 5), (5, 2), (1, 1), (1, 2)])
...
@pytest.mark.parametrize("i,j", [(-5, 5), (5, 2), (1, 1)])
...
@pytest.mark.parametrize(
    "cut_sites,circular,expected_num_fragments",
    [
        ([10], False, 2),
        ([10], True, 1),
        ([10, 15, 20], False, 4),
...
def test_cutting(cut_sites, circular, expected_num_fragments, test_str):
    ...
    assert linkedlist.cyclic == circular, "fragment should be cyclic={}".format(
        circular
    )
...
@pytest.mark.parametrize(
    "query_test_str,expected,template_circular,query_circular",
    [
        ("ABC", (1, 6), False, False),
        ("ABCD", (1,), False, False),
        ("EABC", (5,), False, False),
...
def test_find_iter(
    query_test_str, expected, template_circular, query_circular, direction
):
...
def test_find_iter_min_length(length, expected, direction):
    ...
    found = list(
        template.find_iter(
            DoubleLinkedList("ABCDEFZZZ"), min_query_length=length, direction=direction
        )
    )
...
def test_find_iter_min_length_reverse():
    ...
    found = list(
        template.find_iter(
            DoubleLinkedList("ZZZABCDEF"), min_query_length=1, direction=-1
        )
    )
...
def test_find_iter_protocol():
    ...
    found = list(
        template.find_iter(DoubleLinkedList("BCD"), protocol=lambda x, y: False)
    )
    assert len(found) == 0
...
def test_find_iter_reverse_query():
    ...
    found = list(
        template.find_iter(
            DoubleLinkedList("ZZIJKL"[::-1]), min_query_length=2, direction=(-1, 1)
        )
    )
    assert len(found) == 1
...
@pytest.mark.parametrize("circular", [True, False, True, False])
...
class TestLinkedListMagic(object):
    ...
    def test_splice_magic(self, i, j, circular, linked_list, test_str):
        ...
        if test_str[i:j] == "":
            assert linked_list[i:j].is_empty(), "({},{}) should return None".format(
                i, j
            )
        else:
            assert str(linked_list[i:j]) == test_str[i:j]

    @pytest.mark.parametrize("i,j", [(5, 2), (0, 0), (2, 2), (-2, -4)])
