...
@pytest.mark.parametrize(
    "seq,expected", [["AGTC", "TCAG"[::-1]], ["aaaAAAAaaaaa", "tttTTTTttttt"[::-1]]]
)
def test_dna_reverse_complement(seq, expected):
    assert alphabet.AmbiguousDNA.reverse_complement(seq) == expected
    assert alphabet.AmbiguousDNA.rc(seq) == alphabet.AmbiguousDNA.reverse_complement(
        seq
    )
...
@pytest.mark.parametrize(
    "seq,expected",
    [["AGTC", "TCAG"], ["aaaAAAAaaaaa", "tttTTTTttttt"], ["AGTCN", "TCAGN"]],
)
