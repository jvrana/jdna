...
@pytest.mark.parametrize(
    "primer1_inset",
    [pytest.param(30, id="inset1=30bp"), pytest.param(15, id="inset1=15bp")],
)
@pytest.mark.parametrize(
    "primer2_inset",
    [pytest.param(30, id="inset2=30bp"), pytest.param(15, id="inset2=15bp")],
)
...
def test_pcr(primer1_inset, primer2_inset, o1, o2, circular_template):
    ...
    assert len(products[0]) == expected_len, "Product should be {} long".format(
        expected_len
    )
...
def test_pcr_expected_sequence():
    ...
    assert str(products[0]) == str(
        overhang1 + anneal1 + buffer + anneal2 + overhang2.copy().reverse_complement()
    )
